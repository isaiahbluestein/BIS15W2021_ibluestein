...
```r
#it seems that NAs are represented by -999 
```

```r
summary(amniota)#how can the max weaning weight be greater than the max body weight??  
```
...
## 1 Mamm… Ceta… Balae… Bala… muscul…       -999 Blue Whale             1960.
...
```r
#okay, whales are big, that makes sense!
```
...
```r
summary(amphibio) #looks like NA and 1 is being used as NAs???
```
...
```r
#wait no i think the 1 is a presence/absence in a certain habitat
```
...
```r
#There are NAs, but they just aren't recognized
```
...
##  NA's   :20860
```
There shouldn't be any negative values in female_maturity_d, let's try that again! 
...
```r
#this makes sense since mammals don't lay eggs. It tells me that some columns may be all NA simply because that category doesn't apply to a certain biological group. 
```
...
## 4 Ter        1104     16.3
```
The NAs for these variables don't represent actual missing data, but rather show that the organism isn't classified as that type group. 
